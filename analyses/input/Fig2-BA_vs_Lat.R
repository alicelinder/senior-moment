...
summary(lm2 <- lm(relative.BA ~ minLatdiff, data = focal.centroid[focal.centroid$sp == "BETPAP",]))
summary(lm3 <- lm(relative.BA ~ minLatdiff, data = focal.centroid[focal.centroid$sp == "CORALT",]))
summary(lm4 <- lm(relative.BA ~ minLatdiff, data = focal.centroid[focal.centroid$sp == "FAGGRA",]))
summary(lm5 <- lm(relative.BA~ minLatdiff, data = focal.centroid[focal.centroid$sp == "HAMVIR",]))
#summary(lm6 <- lm(relative.BA ~ minLatdiff, data = focal.centroid[focal.centroid$sp == "QUEALB",]))
summary(lm7 <- lm(relative.BA ~ minLatdiff, data = focal.centroid[focal.centroid$sp == "SORAME",]))
...
stop("The rest of this code is old -- pre Feb 2017 -- and should only be looked at if necessary.")
